...
Full implementation of the researched ATS pipeline (`ats-cv-scoring-research.md`) in Python —
**one file per method**, organized by research section. This variant uses **TF-IDF +
cosine similarity** (scikit-learn) for the semantic-matching tier: zero model downloads,
deterministic, fast. The sister project `ats-engine-transformer` implements the identical
API with sentence-transformers.
...
| `jd_parser.py` | JD structuring: required/preferred skills, min years, quals list |

### §3 Entity normalization — `ats/normalization/`
| File | Research point |
|---|---|
| `taxonomy.py` | skills taxonomy / canonical entities (Workday Skills Cloud style) |
| `synonym_normalizer.py` | CPA ↔ Certified Public Accountant (legacy Taleo doesn't do this) |
...
| `embedding_match.py` | ★ TF-IDF + cosine (this project) / sentence-transformers (sister project) |
| `relationship_clusters.py` | related-concept clusters (Workday HiredScore) |
| `llm_qualification.py` | per-qualification Fit & Gap with evidence; "undecided" without API key |
...
progression, tenure) · `section_placement.py` · `competency_ratings.py` (Taleo model) ·
`engagement_signals.py` (Phenom) · `historical_patterns.py` (Eightfold)

### §7 Adjacent systems — `ats/adjacent/`
`rediscovery.py` (silver medalists) · `skills_assessment.py` (pipeline-stage quizzes) ·
`chatbot_prescreen.py` (conversational knockouts) · `video_interview.py` (HireVue-style
transcript competency scoring, Top/Middle/Bottom bands, BARS anchors) ·
`psychometrics.py` (pymetrics-style trait benchmark matching) ·
`sourcing_rank.py` (LinkedIn Recruiter personalized relevance: query match + team
context + similar-search + history - spam penalty)

### §8 Legal compliance — `ats/compliance/`
`anonymizer.py` (Lever: strip PII/demographics/military before LLM) ·
`bias_audit.py` (4/5ths disparate-impact rule) · `human_review_router.py`
(AI never rejects; opt-out → manual review) · `protected_attribute_guard.py`
(Greenhouse-style: blocks/warns on protected-attribute proxies in JDs)

### Pipeline & review
`ats/pipeline.py` (orchestrator) · `ats/review/review_queue.py` (tiered recruiter queue) ·
`ats/models.py` (shared dataclasses) · `ats/llm_client.py` (optional OpenAI-compatible adapter)
...
Without a key, LLM-based criteria return **"undecided"** (Ashby behavior) and route to
human review — never a fabricated rejection.
...
## Notes
- OCR requires the [tesseract](https://github.com/UB-Mannheim/tesseract/wiki) and poppler
  binaries on Windows; the engine degrades gracefully without them.
- Research note: real enterprise ATS do NOT compute a single 0-100 score — this engine
  provides one because consumers expect it, but the honest outputs are the tiers,
  grades, stars, and criteria counts.
